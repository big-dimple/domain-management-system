...
router.post('/settings', async (req, res) => {
  try {
    ...
    // MongoDB配置
    envContent += '# MongoDB配置\n';
    envContent += `MONGO_USER=${newConfig.database.MONGO_USER}\n`;
    if (newConfig.database.MONGO_PASSWORD && newConfig.database.MONGO_PASSWORD !== '******') {
      envContent += `MONGO_PASSWORD=${newConfig.database.MONGO_PASSWORD}\n`;
    } else {
      envContent += `MONGO_PASSWORD=${process.env.MONGO_PASSWORD}\n`;
    }
    envContent += `MONGODB_URI=mongodb://${newConfig.database.MONGO_USER}:${newConfig.database.MONGO_PASSWORD !== '******' ? newConfig.database.MONGO_PASSWORD : process.env.MONGO_PASSWORD}@mongodb:27017/domains?authSource=admin\n\n`;
    
    // 服务配置
    envContent += '# 服务配置\n';
    envContent += 'NODE_ENV=production\n';
    envContent += `PORT=${newConfig.general.PORT}\n`;
    envContent += `LOG_LEVEL=${newConfig.general.LOG_LEVEL}\n\n`;
    
    // CSV配置
    envContent += '# CSV导入导出配置\n';
    envContent += `CSV_MAX_SIZE=${newConfig.general.CSV_MAX_SIZE}\n`;
    envContent += `CSV_ENCODING=${newConfig.general.CSV_ENCODING}\n\n`;
    
    // 域名扫描配置
    envContent += '# 域名扫描配置\n';
    envContent += `SCAN_ENABLED=${newConfig.scan.SCAN_ENABLED}\n`;
    envContent += `SCAN_CRON=${newConfig.scan.SCAN_CRON}\n`;
    envContent += `SCAN_TIMEOUT=${newConfig.scan.SCAN_TIMEOUT}\n`;
    envContent += `SCAN_BATCH_SIZE=${newConfig.scan.SCAN_BATCH_SIZE}\n\n`;
    
    // SSL证书扫描配置
    envContent += '# SSL证书扫描配置\n';
    envContent += `SSL_SCAN_ENABLED=${newConfig.ssl.SSL_SCAN_ENABLED}\n`;
    envContent += `SSL_SCAN_CRON=${newConfig.ssl.SSL_SCAN_CRON}\n`;
    envContent += `SSL_SCAN_TIMEOUT=${newConfig.ssl.SSL_SCAN_TIMEOUT}\n`;
    envContent += `SSL_SCAN_BATCH_SIZE=${newConfig.ssl.SSL_SCAN_BATCH_SIZE}\n\n`;
    
    // 告警配置
    envContent += '# 告警配置\n';
    envContent += `ALERT_ENABLED=${newConfig.alert.ALERT_ENABLED}\n`;
    envContent += `ALERT_CRON=${newConfig.alert.ALERT_CRON}\n\n`;
    
    // 评估配置
    envContent += '# 评估配置\n';
    envContent += `EVAL_URGENT_DAYS=${newConfig.advanced.EVAL_URGENT_DAYS}\n`;
    envContent += `EVAL_SUGGEST_DAYS=${newConfig.advanced.EVAL_SUGGEST_DAYS}\n`;
    envContent += `EVAL_ATTENTION_DAYS=${newConfig.advanced.EVAL_ATTENTION_DAYS}\n\n`;
    
    // SSL证书评估配置
    envContent += '# SSL证书评估配置\n';
    envContent += `SSL_CRITICAL_DAYS=${newConfig.ssl.SSL_CRITICAL_DAYS}\n`;
    envContent += `SSL_WARNING_DAYS=${newConfig.ssl.SSL_WARNING_DAYS}\n`;
    envContent += `SSL_ATTENTION_DAYS=${newConfig.ssl.SSL_ATTENTION_DAYS}\n`;
